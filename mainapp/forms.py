...
class RegistrationForm(forms.ModelForm):
	...
	def clean_email(self):
		...
		if domain in ['ru']:
			raise forms.ValidationError(f'Регистрация дла домена "{domain}" невозможна')
		if User.objects.filter(email=email).exists():
			raise forms.ValidationError(f'Данная электронная почта уже зарегестрирована')
		return email

	def clean_username(self):
		username = self.cleaned_data['username']
		if User.objects.filter(username=username).exists():
			raise forms.ValidationError(f'Данный логин {username} занят')
		return username
